...
- **8 种运动方式** —— 随机散开、径向爆炸、径向收敛、重力下落、垂直交叉、水平交叉、旋转漩涡、直接变形
- **三种导出格式** —— GIF / WebM / MP4，全部在浏览器内完成，无需上传或装软件
- **导出即所见** —— 预览与导出共用同一套渲染引擎，逐帧一致
- **液态玻璃界面** —— 深 / 浅色双主题，自定义下拉与实时滑块
...
|------|------|
| `npm run dev` | 启动开发服务器（Vite HMR） |
| `npm test` | 运行单元测试（Vitest，59 个用例，纯核心逻辑） |
...
使用 Vitest 对纯核心模块运行单元测试，覆盖采样网格生成、粒子插值与各运动方式、缓动函数、帧序列器、配置状态管理、文件名清洗。共 **59 个测试用例**，全部在 Node.js 环境下运行，无需浏览器。
